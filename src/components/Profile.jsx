    ...
    return (
        <div>
            ...
            <div className='max-w-4xl mx-auto bg-white border border-gray-200 rounded-2xl my-8 p-8 shadow-lg'>
                <div className='flex justify-between'>
                    ...
                    <div className='flex items-center gap-6'>
                        <Avatar className="h-32 w-32 border-2 border-gray-300 shadow-lg">
                            <AvatarImage src={user?.profile?.profilePhoto} alt="Profile Picture" className="rounded-full" />
                        </Avatar>
                        <div>
                            <h1 className='font-semibold text-2xl text-[#333333]'>{user?.fullname}</h1>
                            <p className='text-md text-gray-600'>{user?.profile?.bio}</p>
                        </div>
                    </div>

                    {/* Edit Button */}
                    <Button onClick={() => setOpen(true)} className="text-right text-blue-700 border border-blue-700 rounded-md px-4 py-2 hover:bg-blue-50 transition-all">
                    ...
                    <div className='flex items-center gap-3 flex-wrap'>
                        {
                            user?.profile?.skills.length !== 0 ?
                                user?.profile?.skills.map((item, index) =>
                                    <span key={index} className="text-white font-bold py-1 px-4 rounded-full bg-blue-600">
                                        {item}
                                    </span>
                                )
                ...
                <div className='my-6'>
                    <div className='grid w-full max-w-sm'>
                        ...
                        {
                            isResume ? 
                                <a target='_blank' href={user?.profile?.resume} className='text-blue-600 hover:underline'>
            ...
            <div className='max-w-4xl mx-auto bg-white rounded-2xl my-8 p-8 shadow-lg'>
                <h1 className='font-semibold text-xl text-[#333333] mb-5'>Applied Jobs</h1>
                <AppliedJobTable />
            </div>
